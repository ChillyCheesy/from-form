...
import { FrmControlConfig, FrmController, Operable, UpdateValueFn } from '../models';

export class FrmSelectorController<T> implements FrmController<T> {
  private _value$$: BehaviorSubject<T | undefined> = new BehaviorSubject<T | undefined>(undefined);
  private _options$$: BehaviorSubject<T[]> = new BehaviorSubject<T[]>([]);
  ...
  public writeValue(value: T | undefined): void {
    const currentOptions: T[] = this._options$$.getValue();
  ...
  public updateOptions(updateFn: UpdateValueFn<T[]>): void {
    this.setOptions(updateFn(this._options$$.getValue()));
  }
